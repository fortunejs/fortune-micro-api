  ...
  return Promise.all(options.contexts.concat(contextURI).map(uri =>
    new Promise((resolve, reject) =>
      http.get(Object.assign(url.parse(uri), {
        ...
        // If the server doesn't respond, just time out.
...
function mapRecord (type, record) {
  ...
  for (let i = 0, j = unionFields.length; i < j; i++) {
    ...
    // Detect and expand relative links.
    if (record[field] &&
      typeof record[field][hrefField] === 'string' &&
      record[field][hrefField].charAt(0) === '/')
      record[field][hrefField] =
        `${entryPoint}${record[field][hrefField]}`
