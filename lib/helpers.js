...
import { reservedKeys, inBrackets, vocabulary, namespace, namespaceDelimiter,
  queryDelimiter, isField, isMatch } from './settings'
...
export function initializeContext (context, request) {
  const {
    uriTemplate, methodMap, options, recordTypes, adapter, keys,
...
export function validateContext (payload) {
  const {
    ...
    options: { vocabulary: vocab, namespaces }
  } = this
...
export function mapRecord (type, record) {
  const { keys, recordTypes, uriTemplate,
    options: { inflectType, inflectPath, obfuscateURIs, namespaceMap }
  } = this
...
export function extractNamespace (key, namespaceMap) {
  const { errors: { BadRequestError } } = this
  let namespace
