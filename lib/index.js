  ...
  class MicroApiSerializer extends HttpSerializer {
    ...
    showIndex (contextResponse, request, response) {
      ...
      for (const type in recordTypes) {
        ...
        if (type in documentation) {
          // If documentation is explicitly falsy, don't reveal it.
          if (!documentation[type]) continue
        ...
        const typeFields = Object.getOwnPropertyNames(recordTypes[type])

        for (let i = 0, j = typeFields.length; i < j; i++) {
          const field = typeFields[i]
          const definition = recordTypes[type][field]
          const descriptor =
            Object.getOwnPropertyDescriptor(recordTypes[type], field)

          if (!descriptor.enumerable && !definition.inputOnly) continue
    ...
    parseUpdate (contextRequest) {
      ...
      return (payload[reservedKeys.graph] || [ payload ]).map(update => {
        ...
        let updateId = update[idField]
        if (!updateId && ids.length === 1) updateId = ids[0]

        const id = options.castId ?
          castToNumber(updateId) : updateId
