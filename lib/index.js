...
import { mediaType, reservedKeys, defaults, namespace } from './settings'
import { initializeContext, showQueries, castId, showContext, validateContext,
  mapRecord, attachIncluded, encodeRoute, parseBuffer } from './helpers'


const idField = `${namespace}:${reservedKeys.identifier}`
const metaField = `${namespace}:${reservedKeys.meta}`
const errorField = `${namespace}:${reservedKeys.error}`
...
class MicroApiSerializer extends Serializer {
  ...
  showIndex (context) {
    const { recordTypes, uriTemplate, options,
      options: { inflectPath, inflectType, obfuscateURIs }
    } = this
  ...
  parseCreate (context) {
    ...
    return payload[reservedKeys.graph].map(record => {
      ...
      if (record[reservedKeys.type] !== compareType)
  ...
  parseUpdate (context) {
    ...
    return payload[reservedKeys.graph].map(update => {
      ...
      if (update[reservedKeys.type] !== compareType)
        throw new BadRequestError(
          `The field "${reservedKeys.type}" must be valued ` +
          `as "${compareType}".`)
